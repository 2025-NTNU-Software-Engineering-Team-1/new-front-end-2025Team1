   ...
   */
	public loadAssets(dir: string, fileName: string): void {
		...
		LAppPal.printMessage(
			`[APP] LAppModel.loadAssets 被呼叫 dir=${this._modelHomeDir}, file=${fileName}`
		);
		...
		fetch(url)
			.then((response) => {
				if (!response.ok) {
					// HTTP 狀態錯誤也印出來
					CubismLogError(
						`[APP] Failed to fetch ${url}, status=${response.status}`
					);
				}
			...
			.catch((error) => {
				// model3.json 讀取失敗
				CubismLogError(
					`Failed to load file ${url}, error=${(error as Error).message}`
				);
			});
	}
