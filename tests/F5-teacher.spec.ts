...
test.skip("Post solved", async ({ page }) => {
  ...
  await expect(moreBtn).toBeVisible();
  await moreBtn.click({ force: true });
  const solveItem = page.locator(
    "ul.dropdown-content >> text=discussion.components.solved.action_Not_Solved",
  );
  ...
  const unsolveItem = page.locator(
    "ul.dropdown-content >> text=discussion.components.solved.action_Is_Solved",
  );
