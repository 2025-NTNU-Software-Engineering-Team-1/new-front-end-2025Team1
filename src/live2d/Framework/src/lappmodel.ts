...
export class LAppModel extends CubismUserModel {
  ...
  public loadAssets(dir: string, fileName: string): void {
    ...
    // 除錯用：看實際組出來的路徑
    LAppPal.printMessage(`[APP] LAppModel.loadAssets 被呼叫 dir=${this._modelHomeDir}, file=${fileName}`);

    const url = `${this._modelHomeDir}${fileName}`;
    LAppPal.printMessage(`[APP] 嘗試載入 model3.json: ${url}`);

    fetch(url)
      .then((response) => {
        if (!response.ok) {
          // HTTP 狀態錯誤也印出來
          CubismLogError(`[APP] Failed to fetch ${url}, status=${response.status}`);
        }
      ...
      .then((arrayBuffer) => {
        ...
        LAppPal.printMessage("[APP] setupModel 完成");
      })
      .catch((error) => {
        // model3.json 讀取失敗
        CubismLogError(`Failed to load file ${url}, error=${(error as Error).message}`);
      });
  }
  ...
  private setupTextures(): void {
    // iPhone でのアルファ品質向上のため Typescript では premultipliedAlpha を採用
    ...
    if (this._state == LoadStep.LoadTexture) {
      ...
      for (let modelTextureNumber = 0; modelTextureNumber < textureCount; modelTextureNumber++) {
        ...
        // ロード完了時に呼び出すコールバック関数
        const onLoad = (textureInfo: TextureInfo | null): void => {
          if (!textureInfo) {
            console.error("[APP] テクスチャ読み込み失敗:", texturePath);
            return;
          }
        ...
        this._subdelegate
          .getTextureManager()
          .createTextureFromPngFile(texturePath, gl, usePremultiply, onLoad);
